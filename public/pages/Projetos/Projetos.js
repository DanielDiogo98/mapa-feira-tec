...
async function carregarProjetosDaApi() {
    try {
        const resposta = await fetch('/api/projects');
        if (!resposta.ok) return;
        const corpo = await resposta.json();
        if (!Array.isArray(corpo.data) || corpo.data.length === 0) return;

        projetos = corpo.data.map(projeto => ({
            nome: projeto.name,
            subtitulo: projeto.description || 'Projeto da Feira Tecnológica',
            curso: (projeto.courses || []).join(', ') || 'Curso a confirmar',
            sala: projeto.location?.label || 'A confirmar',
            bloco: projeto.location?.mapId?.includes('bloco-b')
                ? 'B'
        ...
        // Mantém os exemplos enquanto a API ainda não estiver disponível.
    }
}

carregarProjetosDaApi();
